...
public class OrderConstantUtil {
	...
	public static final String SQL_ORDERS_MATERIAL_EMPENHO = 
			"SELECT  "
					...
					+ "        OPI.StOrdemItem,  "
					+ "        Rtrim(OPI.IdPosicao) as IdPosicao, "
					...
					+ "        ISNULL((SELECT SUM(QtMovimento)  "
					+ "         FROM wpcp.OrdemProducao_MovEstoque  "
					+ "         WHERE IdOrdemProducaoItem = OPI.IdOrdemProducaoItem  "
					+ "           AND TpMovimento = 'E'),0) AS QtEmpenhada, "
					+ "        ISNULL((SELECT SUM(QtMovimento)  "
					+ "         FROM wpcp.OrdemProducao_MovEstoque  "
					+ "         WHERE IdOrdemProducaoItem = OPI.IdOrdemProducaoItem  "
					+ "           AND TpMovimento = 'S'),0) AS QtProduzida, "
					+ "        RTRIM(VPP.CdSigla) AS CdSigla"						
					+ "    FROM  "
					...
					+ "            WHEN VPP.TpProduto = 'P' THEN CFG.IdSetorProdutoPersonalizado "
					+ "        END "
					+ "    ) Where OP.StOrdem in ('L','T') "
					+ ") T "
					+ "WHERE  "
					+ "    T.CdEmpresa = :cdEmpresa  ";
//					+ "    AND (T.DtCadastro BETWEEN :startDate AND :endDate) "
//					+ "    AND (T.StOrdemItem = CASE WHEN :stOrdem = '' THEN T.StOrdemItem ELSE :stOrdem END) "					
//					+ "    AND (T.NmProduto LIKE :value "
//					+ "         OR T.CdChamada LIKE :value "
//					+ "         OR T.CdProduto LIKE :value) AND T.IdOrdemProducao NOT IN (:ordensNotIn) "
//					+ ""			;
//	
